...
{
  gpio_set_direction((gpio_num_t)TFT_PIN_MOSI, GPIO_MODE_INPUT);
  #ifdef CONFIG_IDF_TARGET_ESP32
    pinMatrixInAttach(TFT_PIN_MOSI, VSPIQ_IN_IDX, false);
  #else // S2
    pinMatrixInAttach(TFT_PIN_MOSI, FSPIQ_IN_IDX, false);
  #endif
  SET_BUS_READ_MODE;
}
...
{
  gpio_set_direction((gpio_num_t)TFT_PIN_MOSI, GPIO_MODE_OUTPUT);
  #ifdef CONFIG_IDF_TARGET_ESP32
    pinMatrixOutAttach(TFT_PIN_MOSI, VSPID_OUT_IDX, false, false);
  #else // S2
    pinMatrixOutAttach(TFT_PIN_MOSI, FSPID_OUT_IDX, false, false);
  #endif
  SET_BUS_WRITE_MODE;
}
