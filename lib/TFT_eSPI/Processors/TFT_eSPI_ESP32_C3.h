...
#else
  #if defined (TFT_PARALLEL_8_BIT)
    // TFT_PIN_DC, by design, must be in range 0-31 for single register parallel write
    #if (TFT_PIN_DC >= 0) &&  (TFT_PIN_DC < 32)
      #define DC_C GPIO.out_w1tc.val = (1 << TFT_PIN_DC)
      #define DC_D GPIO.out_w1ts.val = (1 << TFT_PIN_DC)
    #elif (TFT_PIN_DC >= 32)
      #define DC_C GPIO.out_w1tc.val = (1 << (TFT_PIN_DC- 32))
      #define DC_D GPIO.out_w1ts.val = (1 << (TFT_PIN_DC- 32))
    #else
      #define DC_C
      #define DC_D
    #endif
        ...
        #define DC_C GPIO.out_w1ts.val = (1 << (TFT_PIN_DC - 32)); \
                     GPIO.out_w1tc.val = (1 << (TFT_PIN_DC - 32))
        #define DC_D GPIO.out_w1tc.val = (1 << (TFT_PIN_DC - 32)); \
                     GPIO.out_w1ts.val = (1 << (TFT_PIN_DC - 32))
      #else
        #define DC_C GPIO.out_w1tc.val = (1 << (TFT_PIN_DC - 32))//;GPIO.out_w1tc.val = (1 << (TFT_PIN_DC - 32))
        #define DC_D GPIO.out_w1ts.val = (1 << (TFT_PIN_DC - 32))//;GPIO.out_w1ts.val = (1 << (TFT_PIN_DC - 32))
      #endif
    #elif (TFT_PIN_DC >= 0)
          ...
          #define DC_C GPIO.out_w1tc.val = (1 << TFT_PIN_DC); \
                       GPIO.out_w1tc.val = (1 << TFT_PIN_DC)
          #define DC_D GPIO.out_w1tc.val = (1 << TFT_PIN_DC); \
                       GPIO.out_w1ts.val = (1 << TFT_PIN_DC)
        #else
        ...
        #endif
      #else
        #define DC_C GPIO.out_w1tc.val = (1 << TFT_PIN_DC)//;GPIO.out_w1tc.val = (1 << TFT_PIN_DC)
        #define DC_D GPIO.out_w1ts.val = (1 << TFT_PIN_DC)//;GPIO.out_w1ts.val = (1 << TFT_PIN_DC)
      #endif
    #else
      #define DC_C
      #define DC_D
...
////////////////////////////////////////////////////////////////////////////////////////
#ifndef TFT_PIN_CS
  #define TFT_PIN_CS -1  // Keep DMA code happy
  #define CS_L       // No macro allocated so it generates no code
  #define CS_H       // No macro allocated so it generates no code
#else
  #if defined (TFT_PARALLEL_8_BIT)
    #if TFT_PIN_CS >= 32
        #define CS_L GPIO.out_w1tc.val = (1 << (TFT_PIN_CS - 32))
        #define CS_H GPIO.out_w1ts.val = (1 << (TFT_PIN_CS - 32))
    #elif TFT_PIN_CS >= 0
        #define CS_L GPIO.out_w1tc.val = (1 << TFT_PIN_CS)
        #define CS_H GPIO.out_w1ts.val = (1 << TFT_PIN_CS)
    #else
      #define CS_L
      #define CS_H
    #endif
        ...
        #define CS_L GPIO.out_w1ts.val = (1 << (TFT_PIN_CS - 32)); \
                     GPIO.out_w1tc.val = (1 << (TFT_PIN_CS - 32))
        #define CS_H GPIO.out_w1tc.val = (1 << (TFT_PIN_CS - 32)); \
                     GPIO.out_w1ts.val = (1 << (TFT_PIN_CS - 32))
      #else
        #define CS_L GPIO.out_w1tc.val = (1 << (TFT_PIN_CS - 32)); GPIO.out_w1tc.val = (1 << (TFT_PIN_CS - 32))
        #define CS_H GPIO.out_w1ts.val = (1 << (TFT_PIN_CS - 32))//;GPIO.out_w1ts.val = (1 << (TFT_PIN_CS - 32))
      #endif
    #elif (TFT_PIN_CS >= 0)
      #ifdef RPI_DISPLAY_TYPE  // RPi display needs a slower CS change
        #define CS_L GPIO.out_w1ts.val = (1 << TFT_PIN_CS); GPIO.out_w1tc.val = (1 << TFT_PIN_CS)
        #define CS_H GPIO.out_w1tc.val = (1 << TFT_PIN_CS); GPIO.out_w1ts.val = (1 << TFT_PIN_CS)
      #else
        #define CS_L GPIO.out_w1tc.val = (1 << TFT_PIN_CS); GPIO.out_w1tc.val = (1 << TFT_PIN_CS)
        #define CS_H GPIO.out_w1ts.val = (1 << TFT_PIN_CS)//;GPIO.out_w1ts.val = (1 << TFT_PIN_CS)
      #endif
    #else
      #define CS_L
      #define CS_H
...
////////////////////////////////////////////////////////////////////////////////////////
      ...
      #if (TFT_PIN_MISO == -1)
        #undef TFT_PIN_MISO
        #define TFT_PIN_MISO TFT_PIN_MOSI
      #endif
    #endif
