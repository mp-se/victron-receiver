...
////////////////////////////////////////////////////////////////////////////////////////
  ...
  // Select the SPI port and board package to use
    ...
    MbedSPI spi = MbedSPI(TFT_PIN_MISO, TFT_PIN_MOSI, TFT_PIN_SCLK);
  #else
    // Community RP2040 board package by Earle Philhower
    //SPIClass& spi = SPI; // will use board package default pins
    SPIClassRP2040 spi = SPIClassRP2040(SPI_X, TFT_PIN_MISO, -1, TFT_PIN_SCLK, TFT_PIN_MOSI);
  #endif
...
uint8_t TFT_eSPI::tft_Read_8(void)
{
  ...
  for (uint8_t i = 0; i < 8; i++) {  // read results
    ret <<= 1;
    SCLK_L;
    if (digitalRead(TFT_PIN_MOSI)) ret |= 1;
    SCLK_H;
  }
  */
...
void pioinit(uint32_t clock_freq) {
  ...
  pio_gpio_init(tft_pio, TFT_PIN_DC);
  pio_gpio_init(tft_pio, TFT_PIN_SCLK);
  pio_gpio_init(tft_pio, TFT_PIN_MOSI);

  // Configure the pins to be outputs
  pio_sm_set_consecutive_pindirs(tft_pio, pio_sm, TFT_PIN_DC, 1, true);
  pio_sm_set_consecutive_pindirs(tft_pio, pio_sm, TFT_PIN_SCLK, 1, true);
  pio_sm_set_consecutive_pindirs(tft_pio, pio_sm, TFT_PIN_MOSI, 1, true);

  // Configure the state machine
  pio_sm_config c = tft_io_program_get_default_config(program_offset);

  sm_config_set_set_pins(&c, TFT_PIN_DC, 1);
  // Define the single side-set pin
  sm_config_set_sideset_pins(&c, TFT_PIN_SCLK);
  // Define the pin used for data output
  sm_config_set_out_pins(&c, TFT_PIN_MOSI, 1);
