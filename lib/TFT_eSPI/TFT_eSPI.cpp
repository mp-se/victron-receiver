...
void TFT_eSPI::initBus(void) {
  ...
  if (TFT_PIN_CS >= 0) {
    ...
    digitalWrite(TFT_PIN_CS, HIGH); // Chip select high (inactive)
  }
#endif
...
// In parallel mode and with the RP2040 processor, the TFT_WR line is handled in the  PIO
  ...
  if (TFT_PIN_DC >= 0) {
    ...
    digitalWrite(TFT_PIN_DC, HIGH); // Data/Command high = data mode
  }
#endif
  ...
  if (TFT_PIN_RST >= 0) {
    ...
    digitalWrite(TFT_PIN_RST, HIGH); // Set high, do not share pin with another SPI device
  }
#endif
...
void TFT_eSPI::init(uint8_t tc)
{
  ...
  #if defined (TFT_PIN_CS) && (TFT_PIN_CS >= 0)
    cspinmask = (uint32_t) digitalPinToBitMask(TFT_PIN_CS);
  #endif

  #if defined (TFT_PIN_DC) && (TFT_PIN_DC >= 0)
    dcpinmask = (uint32_t) digitalPinToBitMask(TFT_PIN_DC);
  #endif
  ...
  #if defined (TFT_PIN_SCLK) && (TFT_PIN_SCLK >= 0)
    sclkpinmask = (uint32_t) digitalPinToBitMask(TFT_PIN_SCLK);
  #endif
...
#else
  #if !defined(TFT_PARALLEL_8_BIT) && !defined(RP2040_PIO_INTERFACE)
    #if defined (TFT_PIN_MOSI) && !defined (TFT_SPI_OVERLAP) && !defined(ARDUINO_ARCH_RP2040) && !defined (ARDUINO_ARCH_MBED)
      spi.begin(TFT_PIN_SCLK, TFT_PIN_MISO, TFT_PIN_MOSI, -1); // This will set MISO to input
    #else
...
#if defined (TFT_PIN_CS) && !defined(RP2040_PIO_INTERFACE)
...
#if defined (TFT_PIN_DC) && !defined(RP2040_PIO_INTERFACE)
  if (TFT_PIN_DC >= 0) {
    pinMode(TFT_PIN_DC, OUTPUT);
    digitalWrite(TFT_PIN_DC, HIGH); // Data/Command high = data mode
  }
  ...
  // Toggle RST low to reset
    ...
    // Set to output once again in case MISO is used for TFT_PIN_RST
    if (TFT_PIN_RST >= 0) {
      pinMode(TFT_PIN_RST, OUTPUT);
    }
  #endif
...
#if defined (TFT_PIN_MOSI)
  tft_settings.pin_TFT_PIN_MOSI = TFT_PIN_MOSI;
#else
  tft_settings.pin_TFT_PIN_MOSI = -1;
#endif

#if defined (TFT_PIN_MISO)
  tft_settings.pin_TFT_PIN_MISO = TFT_PIN_MISO;
#else
  tft_settings.pin_TFT_PIN_MISO = -1;
#endif

#if defined (TFT_PIN_SCLK)
  tft_settings.pin_tft_clk  = TFT_PIN_SCLK;
#else
  tft_settings.pin_tft_clk  = -1;
#endif

#if defined (TFT_PIN_CS)
  tft_settings.pin_TFT_PIN_CS   = TFT_PIN_CS;
#else
  tft_settings.pin_TFT_PIN_CS   = -1;
#endif

#if defined (TFT_PIN_DC)
  tft_settings.pin_TFT_PIN_DC  = TFT_PIN_DC;
#else
  tft_settings.pin_TFT_PIN_DC  = -1;
#endif
...
#if defined (TFT_PIN_RST)
  tft_settings.pin_TFT_PIN_RST = TFT_PIN_RST;
#else
  tft_settings.pin_TFT_PIN_RST = -1;
#endif
