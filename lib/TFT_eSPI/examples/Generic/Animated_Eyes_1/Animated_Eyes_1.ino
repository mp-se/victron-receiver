...
struct {                // One-per-eye structure
  int16_t   TFT_PIN_CS;     // Chip select pin for each display
...
void setup(void) {
  ...
  digitalWrite(eye[0].TFT_PIN_CS, HIGH);
  if (NUM_EYES > 1) digitalWrite(eye[1].TFT_PIN_CS, HIGH);

  for (uint8_t e = 0; e < NUM_EYES; e++) {
    digitalWrite(eye[e].TFT_PIN_CS, LOW);
    tft.setRotation(eyeInfo[e].rotation);
    tft.fillScreen(TFT_BLACK);
    digitalWrite(eye[e].TFT_PIN_CS, HIGH);
  }
